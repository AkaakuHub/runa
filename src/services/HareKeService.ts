...
const POSITIVE_WORDS = [
	'ありがとう', 'おめでとう', '嬉しい', '楽しい', '面白い', '素晴らしい', '良い', 'いいね',
	'最高', 'すごい', '頑張', '成功', '勝利', '達成', '完成', '解決', '発見', '進歩',
	'笑', 'www', 'w', '😊', '😄', '😆', '🎉', '✨', '👍', '❤️', '💕', '🥰',
	'やった', 'よし', 'おお', 'わーい', 'やったー', 'すげー', 'かっこいい', 'かわいい'
] as const;

const NEGATIVE_WORDS = [
	'疲れた', 'しんどい', 'つらい', '悲しい', '困った', '大変', '厳しい', '辛い',
	'失敗', '問題', 'エラー', 'バグ', '故障', '不具合', '遅延', '中断', '停止',
	'だめ', 'まずい', 'やばい', '心配', '不安', '迷惑', 'うざい', 'むかつく',
	'😢', '😭', '😰', '😓', '💦', '😵', '🤦', '😤', '💔', '😞'
] as const;
 ...
 * メインの判定関数
 */
export const judgeHareKe = async (messages: MessageData[], targetDate: Date): Promise<HareKeResult> => {
	const breakdown = {
	...
	const totalScore = Math.round(
		...
		breakdown.tradition.score * 0.20 +
 ...
 * 活動度判定 (35%)
 */
function judgeActivity(messages: MessageData[]): { score: number; reason: string } {
 ...
 * 感情度判定 (25%)
 */
function judgeEmotion(messages: MessageData[]): { score: number; reason: string } {
	let positiveCount = 0;
	let negativeCount = 0;
...
function judgeNature(date: Date): { score: number; reason: string } {
	...
	if (dayOfWeek === 5) { // 金曜日
		score += 20;
		reasons.push('金曜日');
	} else if (dayOfWeek === 6 || dayOfWeek === 0) { // 土日
		score += 25;
 ...
 * レベルに応じた絵文字とタイトルを取得
 */
function getLevelInfo(level: HareKeResult['level']): { emoji: string; title: string } {
	switch (level) {
		case 'dai-hare':
 ...
 * レベルに応じたメッセージを生成
 */
function generateMessage(level: HareKeResult['level'], breakdown: HareKeResult['breakdown']): string {
	const messages = {
		'dai-hare': [
	...
	const seed = Object.values(breakdown).reduce((sum, item) => sum + item.score, 0);
